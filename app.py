...
if model is not None:
    if choice == "Input Prediksi Manual":
        ...
        with st.form("my_form"):
            ...
            with col2:
                smt_prop  = st.number_input("Semester Seminar Proposal", 0, 14, 7)
                smt_hasil = st.number_input("Semester Seminar Hasil", 0, 14, 8)
        ...
        if submit:
            if smt_hasil > 0 and smt_prop > 0 and smt_hasil < smt_prop:
                ...
                # Urutan fitur: ['Smt Hasil', 'IPK', 'Smt Prop', 'L(1)/P(2)']
                input_data = [[smt_hasil, ipk, smt_prop, gender]]
                prediction = model.predict(input_data)[0]
                proba      = model.predict_proba(input_data)[0]
 
                st.divider()
                if prediction == 1:  # KTW
                    st.success("HASIL: **KTW (Lulus Tepat Waktu)**")
                    st.info("💡 **Analisis:** Mahasiswa memiliki rekam jejak akademik yang baik dan mengikuti linimasa tugas akhir dengan tepat waktu.")
                else:  # Non-KTW
                    st.warning("HASIL: **Non-KTW (Lulus Terlambat)**")
        ...
        try:
            df_X = pd.DataFrame(X_val, columns=['Smt Hasil', 'IPK', 'Smt Prop', 'L(1)/P(2)'])
            df_y = pd.DataFrame(y_val, columns=['Kelas'])
            df_val = pd.concat([df_X, df_y], axis=1)
 
            for col in ['Smt Hasil', 'Smt Prop', 'L(1)/P(2)', 'Kelas']:
                df_val[col] = df_val[col].astype(int)
 
            df_val['Keterangan'] = df_val['Kelas'].map({1: 'KTW', 0: 'Non-KTW'})
 
            df_display = df_val.rename(columns={
            ...
            kolom_urut = ['Gender L[1]/P[2]', 'IPK', 'Semester Seminar Proposal',
                          'Semester Seminar Hasil', 'Kelas', 'Keterangan']
            st.dataframe(df_display[kolom_urut], use_container_width=True)
