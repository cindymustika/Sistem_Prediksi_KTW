...
if model is not None:
    if choice == "Input Prediksi Manual":
        ...
        if submit:
            if smt_hasil > 0 and smt_prop > 0 and smt_hasil < smt_prop:
                ...
                if prediction == 1: # KTW
                    ...
                    # --- LOGIKA IF-ELSE TAMBAHAN UNTUK DOSEN ---
                    st.write("🔍 **Analisis Faktor Penyebab Potensial:**")
                    
                    alasan = []
                    if smt_hasil > 8:
                        alasan.append(f"- **Seminar Hasil Terlambat:** Mahasiswa baru menempuh Seminar Hasil pada semester {int(smt_hasil)} (melebihi standar masa KTW maksimal semester 8).")
                    if smt_prop > 7:
                        alasan.append(f"- **Seminar Proposal Terlambat:** Pengerjaan tugas akhir/skripsi baru dimulai atau baru diseminarkan pada semester {int(smt_prop)}.")
                    if ipk < 3.00:
                        alasan.append(f"- **IPK Kurang Optimal:** Nilai IPK saat ini ({ipk:.2f}) berada di bawah 3.00, yang berpotensi menghambat persyaratan kelulusan atau mencerminkan adanya kendala akademik pada mata kuliah terdahulu.")
                    if smt_prop == 0 and smt_hasil == 0:
                        alasan.append("- **Belum Memulai Tugas Akhir:** Mahasiswa belum melaksanakan Seminar Proposal maupun Seminar Hasil hingga semester evaluasi saat ini.")
                    
                    # Jika tidak masuk kriteria di atas tapi tetap Non-KTW (berarti murni keputusan pola model)
                    if not alasan:
                        alasan.append("- **Kombinasi Tren Akademik:** Berdasarkan pola data historis, kombinasi nilai IPK dan semester aktif mahasiswa ini cenderung mengarah pada kelulusan Non-KTW.")
                    
                    # Tampilkan semua alasan yang terdeteksi
                    for p in alasan:
                        st.write(p)
        ...
        # 3. UBAH NAMA KOLOM AGAR TIDAK DISINGKAT (Ganti Label Tampilan)
        ...
        # Tampilkan tabel yang sudah rapi dan panjang namanya
        st.dataframe(df_display, use_container_width=True)
        
        st.info("💡 **Tips:** Kamu bisa ambil data dari tabel ini untuk mencoba input manual di menu sebelah kiri.")

else:
    st.error("Model gagal dimuat. Pastikan file joblib sudah di-upload ke GitHub.")
